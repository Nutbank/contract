...
/**
 *Submitted for verification at Etherscan.io on 20
/**
 * @title SafeMath
...
contract Nutbank {
  ...
  address dev_0_master = msg.sender;
  address dev_1_master = msg.sender;
  address private dev_0_escrow = msg.sender;
  address private dev_1_escrow = msg.sender;
  address owner = msg.sender;

  uint256 public constant minInvest = 100 trx;
  ...
  function changeMin(uint minAmt) external{
  ...
  function invest(address _affiliate) public payable {
    require(msg.value >= minInvest, "min 100 tron");
  ...
  function calculateProfit(address _investor) public view returns(uint256) {
    uint256 hourDifference = now.sub(investors[_investor].lastInvestmentTime);
    uint256 rate = percentRateInternal(investors[_investor].lockbox);
    uint256 calculatedPercent = (hourDifference.mul(rate))/ 3600;
  ...
  function percentRateInternal(uint256 _balance) public pure returns(uint256) {
    ...
    uint256 step_1 = 50000 trx;
    uint256 step_2 = 100000 trx;
    uint256 step_3 = 150000 trx;
    uint256 step_4 = 500000 trx;
 

    uint256 dailyPercent_0 = 145; //  3.5%
    uint256 dailyPercent_1 = 205; //  5%
    uint256 dailyPercent_2 = 305; //  7.5%
    uint256 dailyPercent_3 = 406; //  10%
    uint256 dailyPercent_4 = 600; //  15% uint256 dailyPercent_0 = 25;   //  0.6%
  ...
  function percentRatePublic(uint256 _balance) public pure returns(uint256) {
    ...
    uint256 dailyPercent_0 = 135; //  3.5%
    uint256 dailyPercent_1 = 500; //  5%
    uint256 dailyPercent_2 = 750; //  7.5%
    uint256 dailyPercent_3 = 1000; //  10%
    uint256 dailyPercent_4 = 1500; //  15%
